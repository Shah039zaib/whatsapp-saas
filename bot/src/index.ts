...
const HEALTH_PING_INTERVAL_MS = Number(process.env.HEALTH_PING_INTERVAL_MS || 240000);
...
async function startBot() {
  try {
    ...
    sock.ev.on("connection.update", async (update: any) => {
      ...
      if (SERVER_URL) {
        try { await axios.post(`${SERVER_URL}/api/debug/bot-status`, { update, ts: Date.now() }).catch(()=>{}); } catch {}
      }

      if (qr && SERVER_URL) {
        try { await axios.post(`${SERVER_URL}/api/session/qr`, { qr }, { timeout: 5000 }); console.log("Posted QR to server"); } catch (e) { console.warn("Could not post QR:", e?.message || e); }
      }

      if (connection === "open") {
        reconnectAttempts = 0;
        console.log("WA connected");
        if (SERVER_URL) { try { await axios.post(`${SERVER_URL}/api/session/qr/clear`).catch(()=>{}); } catch {} }
      } else if (connection === "close") {
        const code = lastDisconnect?.error?.output?.statusCode;
        console.log("Connection closed, reason:", code);
        if (code === 405) {
          reconnectAttempts++;
          console.warn("405 from server. Backing off attempt:", reconnectAttempts);
