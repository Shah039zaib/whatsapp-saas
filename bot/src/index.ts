...
async function startBot() {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of messages) {
      ...
      // handle images: if message contains image, download and process
      if (msg.message.imageMessage) {
        const media = msg.message.imageMessage;
        // bot should download image and POST to server
        // For simplicity, we do not implement download here; admin will upload via web UI in MVP
      }
